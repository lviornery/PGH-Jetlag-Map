...
var map = new ol.Map({
    ...
    layers: layersList,
    view: new ol.View({
        extent: [-8945735.093717, 4894216.585178, -8870936.445609, 4964513.428257], maxZoom: 28, minZoom: 1
    })
});

//initial view - epsg:3857 coordinates if not "Match project CRS"
map.getView().fit([-8945735.093717, 4894216.585178, -8870936.445609, 4964513.428257], map.getSize());
...
let measuring = false;
	...
	var styleFunction = function (feature, type) {
	  ...
	  if (!type || type === type) {
		if (type === "LineString") {
          ...
          lineString = new ol.geom.LineString([center,edgeCoordinate]);
        }
	  }
	  if (lineString) {
		var count = 0;
  ...
  */
  var formatRadius = function (circle) {
    ...
    var c2 = ol.proj.transform(edgeCoordinate, sourceProj, 'EPSG:4326');
    var length = ol.sphere.getDistance(c1, c2);
    feet_radius = convertToFeet(length)
    var output;
    output = (Math.round(feet_radius / 5280 * 100) / 100) + ' miles';
    return output;
  };
